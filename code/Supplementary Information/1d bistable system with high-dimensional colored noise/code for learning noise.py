...
dt = 0.01; #steplength
TestLength=450# Number of points for test/validation phase: 0.01 steplength
TrainLength= 1000
StartLength = 1980
start = time.time() 
...
def DoubleWell():#Load data
    q=np.load('../data/1d bistable system with high-dimensional colored noise/noise.npy')
    U=q.reshape(1,-1)
    length = np.shape(U)[1]
    plt.plot(U[0,StartLength - TrainLength:StartLength + TestLength],color='coral',linestyle='-')
    plt.show()
...
class Reservoir:
    ...
    def _run(self,WoutPara,i):
        ...
        for index in range(self.NumBifurPara):
            ...
            # np.save('NoiseLearned.npy',self.S[0,:])
...
if __name__ == '__main__':
    ...
    if UseDoubleWell==1:
        ...
        for i in range (L):
            ...
            rcpara[1]=0.996  #D
            rcpara[2]=1.25  #relevant to rho: rho=1
            rcpara[3]=0.048   #alpha
            rcpara[4]=1e-8   #beta
        ...
        # np.save('NoiseTotal.npy',NoiseTotal)  #Save the noise we learnt for code 'Total'
