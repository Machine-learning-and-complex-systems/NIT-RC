...
def draw_distribution(a,b,title,xlable,bins_interval=1,margin=1):#To draw the PDF of evaluation between test and predicted data
    ...
    plt.savefig('up'+title + '.pdf',bbox_inches = 'tight')
    plt.show()
...
def DoubleWell(para): #Generate dataset
    ...
    #plt.savefig('upatra.pdf',bbox_inches = 'tight')
    #plt.show()
...
class Reservoir:
    ...
    def _run(self,WoutPara,Load,Predict):
        ...
        output=[]
        if Load==0:
            np.save(r'../data/1d tilted bistable system/upWinw.npy',self.Win)
            np.save(r'../data/1d tilted bistable system/upAw.npy',self.A)
            np.save(r'../data/1d tilted bistable system/upWoutParaw.npy',WoutPara)
            np.save(r'../data/1d tilted bistable system/upWb.npy',self.Wb)
            np.save(r'../data/1d tilted bistable system/upWxi.npy',self.Wxi)
            np.save(r'../data/1d tilted bistable system/upr.npy',self.rFinalTrainTotal)
            
        else:
            self.Win=np.load(r'../data/1d tilted bistable system/upWinw.npy')
            self.A=np.load(r'../data/1d tilted bistable system/upAw.npy')
            WoutPara=np.load(r'../data/1d tilted bistable system/upWoutParaw.npy')
            self.Wb=np.load(r'../data/1d tilted bistable system/upWb.npy')
            self.Wxi=np.load(r'../data/1d tilted bistable system/upWxi.npy')
            self.rFinalTrainTotal=np.load(r'../data/1d tilted bistable system/upr.npy')
        ...
        for index in range(self.NumBifurPara):
            ...
            #    plt.xlim(0,30)
            #    plt.savefig('upptra.pdf',bbox_inches = 'tight')
            #    plt.show()
            ...
            #plt.savefig('updnoise.pdf',bbox_inches = 'tight')
            #plt.show()
...
if __name__ == '__main__':
    ...
    if UseDoubleWell==1:
        for i in range (1):
            ...
            rcpara[0]=4.2  #K_{in}
            rcpara[1]=4  #D
            rcpara[2]=4300  #relevant to rho: rho=0.0011
            rcpara[3]=0.18  #alpha
            rcpara[4]=1e-8  #beta
        ...
        if Predict == 1:
            ...
            draw_distribution(T_auptimes,T_puptimes, "Transition up times", 'Times',bins_interval=1,margin=2)
            draw_distribution(T_adowntimes,T_pdowntimes, "Transition down times", 'Times',bins_interval=1,margin=2)
            draw_distribution(T_aupinterval, T_pupinterval, "Transition up time step", 'Time step ',bins_interval=50,margin=10)
            draw_distribution(T_adowninterval, T_pdowninterval, "Transition down time step", 'Time step ',bins_interval=50,margin=10)
