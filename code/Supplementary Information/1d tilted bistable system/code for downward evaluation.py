...
def distribution(data,T_uptimes,T_downtimes,T_upinterval,T_downinterval):#To evaluate the transition time and number
            ...
            if state<0:
                if diff[position[i]]>0:
                    if np.max(data[position[i]:position[i+1],0])>=1 :
                        ...
                        T_upinterval.append(index_up-index_down)
                        state=1
                        upcount+=1
                ...
                if diff[position[i]]<0:
                    if np.min(data[position[i]:position[i+1],0])<=-1 : 
                        ...
                        T_downinterval.append(index_down-index_up)
                        state=-1
                        downcount+=1
        else:
            if state<0:
                ...
                if diff[position[i]]<0:
                    if np.min(data[position[i]:,0])<=-1 : 
                        ...
                        state=-1
                        downcount+=1
    T_uptimes.append(upcount)
    T_downtimes.append(downcount)     
...
def DoubleWell(para):#Generate dataset
    dimensions = 1
    a = 0;    b = 5;    c=0.25;    epsilon=0.3#0.1
    ...
    #plt.savefig('downatra.pdf',bbox_inches = 'tight')
    #plt.show()
...
class Reservoir:
    def __init__(self,Utotal,hyperparameters):      
        ...
        self.T2=self.input_len# later revise
    ...
    def _run(self,WoutPara,Load,Predict):
        ...
        output=[]
        if Load==0:
            np.save(r'../data/1d tilted bistable system/downWinw.npy',self.Win)
            np.save(r'../data/1d tilted bistable system/downAw.npy',self.A)
            np.save(r'../data/1d tilted bistable system/downWoutParaw.npy',WoutPara)
            np.save(r'../data/1d tilted bistable system/downWb.npy',self.Wb)
            np.save(r'../data/1d tilted bistable system/downWxi.npy',self.Wxi)
            np.save(r'../data/1d tilted bistable system/downr.npy',self.rFinalTrainTotal)    
        else:
            self.Win=np.load(r'../data/1d tilted bistable system/downWinw.npy')
            self.A=np.load(r'../data/1d tilted bistable system/downAw.npy')
            WoutPara=np.load(r'../data/1d tilted bistable system/downWoutParaw.npy')
            self.Wb=np.load(r'../data/1d tilted bistable system/downWb.npy')
            self.Wxi=np.load(r'../data/1d tilted bistable system/downWxi.npy')
            self.rFinalTrainTotal=np.load(r'../data/1d tilted bistable system/downr.npy')
        ...
        for index in range(self.NumBifurPara):
            ...
            if Predict == 0:
                ...
                #plt.xlim(0,30)
                #plt.savefig('downptra.pdf',bbox_inches = 'tight')
                #plt.show()
            ...
            #plt.savefig('downdnoise.pdf',bbox_inches = 'tight')
            #plt.show()
...
if __name__ == '__main__':
    ...
    if UseDoubleWell==1:
        ...
        if Predict == 1:
            ...
            draw_distribution(T_adowninterval, T_pdowninterval, "Downward transition down time step", 'Time step ',bins_interval=50,margin=10)
