...
def draw_distribution(a,b,title,xlable,bins_interval=1,margin=1):#To draw the PDF of evaluation between test and predicted data
    ...
    plt.savefig('../results/upward_' + title + '.pdf',bbox_inches = 'tight')
    plt.show()
...
def DoubleWell(para): #Generate dataset
    ...
    #plt.show()
    #plt.savefig('upxyatra.svg',bbox_inches = 'tight')
...
class Reservoir:
    ...
    def _run(self,WoutPara,Load,Predict):
        ...
        output=[[],[]]
        if Load==0:
            np.save(r'../data/2d tilted bistable non-gradient system/upxyWinw.npy',self.Win)
            np.save(r'../data/2d tilted bistable non-gradient system/upxyAw.npy',self.A)
            np.save(r'../data/2d tilted bistable non-gradient system/upxyWoutParaw.npy',WoutPara)            
            np.save(r'../data/2d tilted bistable non-gradient system/upxyWb.npy',self.Wb)
            np.save(r'../data/2d tilted bistable non-gradient system/upxyWxi.npy',self.Wxi)
            np.save(r'../data/2d tilted bistable non-gradient system/upxyr.npy',self.rFinalTrainTotal)
        else:
            self.Win=np.load(r'../data/2d tilted bistable non-gradient system/upxyWinw.npy')
            self.A=np.load(r'../data/2d tilted bistable non-gradient system/upxyAw.npy')
            WoutPara=np.load(r'../data/2d tilted bistable non-gradient system/upxyWoutParaw.npy')
            self.Wb=np.load(r'../data/2d tilted bistable non-gradient system/upxyWb.npy')
            self.Wxi=np.load(r'../data/2d tilted bistable non-gradient system/upxyWxi.npy')
            self.rFinalTrainTotal=np.load(r'../data/2d tilted bistable non-gradient system/upxyr.npy')
        ...
        for index in range(self.NumBifurPara):
            ...
            if Predict == 0:
                ...
                #plt.xlim(0,500)
                #plt.ylim(-2,2)
                #plt.savefig('upxdeterministic.pdf',bbox_inches = 'tight')
                #plt.show()
                ...
                #plt.savefig('upxyptra.pdf',bbox_inches = 'tight')
                #plt.show()
                ...
                #plt.savefig('upxptra.pdf',bbox_inches = 'tight')
                #plt.show()
            ...
            #plt.xlabel('$x$',fontsize=12)
            #plt.savefig('upxdnoise.pdf',bbox_inches = 'tight')
            #plt.show()
            #plt.hist(yNoiseToDraw, bins=bins, density=True, color=['coral','seagreen'])
            #plt.legend(['True','Predicted'],fontsize=12,loc='upper right')
            #plt.xlabel('$y$',fontsize=12)
            #plt.savefig('upydnoise.pdf',bbox_inches = 'tight')
            #plt.show()
...
if __name__ == '__main__':
    ...
    if UseDoubleWell==1:
        ...
        for i in range (1):
            ...
            rcpara[0]=1.1   #K_{in}
            rcpara[1]=3   #D
            rcpara[2]=4000   #relevant to rho: rho=0.00091
            rcpara[3]=0.52  #alpha
            rcpara[4]=1e-7 #beta
        ...
        if Predict == 1:                      
            ...
            draw_distribution(T_aupinterval, T_pupinterval, "Upward transition time step", 'Time step',bins_interval=50,margin=10)
