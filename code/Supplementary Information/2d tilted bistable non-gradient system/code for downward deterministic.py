...
dt = 0.001 #steplength
TestLength=25000#00 # Number of points for test/validation phase: 0.01 steplength
T2 =25+TestLength*dt  # Total time length: including training phase and test/validation phase
T_auptimes=[]
T_adowntimes=[]
T_puptimes=[]
T_pdowntimes=[]
...
def distribution(u,uptimes,downtimes,upinterval,downinterval):#To evaluate the transition time and number
    xshape=np.shape(u)[0]
    yshape=np.shape(u)[1]
        ...
        if i < Length - transtep -1:
            if state < 0:
                if u[0,i] <0 and u[0,i+1] >=0: 
                    if (u[0,i+1:i+transtep]>=0).all():
                        index_up=i
                        upcount+=1
                        upinterval.append(index_up-index_down)
                        i+=transtep
                        state=1
                    else:
                        i+=1
                ...
                if u[0,i] >0 and u[0,i+1] <=0:
                    if (u[0,i+1:i+transtep]<=0).all():
                        index_down=i
                        downcount+=1
                        downinterval.append(index_down-index_up)
                        i+=transtep
                        state=-1
                    else:
                        i+=1
            ...
            if state < 0:
                ...
                if u[0,i] >0 and u[0,i+1] <=0: 
                    ...
                    i+=1
    uptimes.append(upcount)
    downtimes.append(downcount)
...
def draw_distribution(a,b,title,xlable,bins_interval=1,margin=1):#To draw the PDF of evaluation between test and predicted data
    ...
    plt.savefig('../results/downward_' + title + '.pdf',bbox_inches = 'tight')
    plt.show()

def DoubleWell(para): #Generate dataset
    dimensions = 2
    a = 2;    b = 5;    c = 0.25;    epsilon = 0.3
    ...
    # plt.scatter(u[0,:int(T2/dt)+1-TestLength],u[1,:int(T2/dt)+1-TestLength],c='coral',s=0.4)
    # plt.xlim(-2,2)
    # plt.ylim(-2,2)
    # plt.xlabel('$x$',fontsize=12)
    # plt.ylabel('$y$',fontsize=12)
    # plt.show()
    # plt.savefig('downxyatra.svg',bbox_inches = 'tight')
    u_t=np.zeros((1,TestLength))
    u_t[0,:]=u[0,25001:int(T2/dt)+1]
    distribution(u_t,T_auptimes,T_adowntimes,T_aupinterval,T_adowninterval)
    return u
...
class Reservoir:
    ...
    def _run(self,WoutPara,Load,Predict):
        ...
        output=[[],[]]
        if Load==0:
            np.save(r'../data/2d tilted bistable non-gradient system/downxyWinw.npy',self.Win)
            np.save(r'../data/2d tilted bistable non-gradient system/downxyAw.npy',self.A)
            np.save(r'../data/2d tilted bistable non-gradient system/downxyWoutParaw.npy',WoutPara)            
            np.save(r'../data/2d tilted bistable non-gradient system/downxyWb.npy',self.Wb)
            np.save(r'../data/2d tilted bistable non-gradient system/downxyWxi.npy',self.Wxi)
            np.save(r'../data/2d tilted bistable non-gradient system/downxyr.npy',self.rFinalTrainTotal)
        else:
            self.Win=np.load(r'../data/2d tilted bistable non-gradient system/downxyWinw.npy')
            self.A=np.load(r'../data/2d tilted bistable non-gradient system/downxyAw.npy')
            WoutPara=np.load(r'../data/2d tilted bistable non-gradient system/downxyWoutParaw.npy')
            self.Wb=np.load(r'../data/2d tilted bistable non-gradient system/downxyWb.npy')
            self.Wxi=np.load(r'../data/2d tilted bistable non-gradient system/downxyWxi.npy')
            self.rFinalTrainTotal=np.load(r'../data/2d tilted bistable non-gradient system/downxyr.npy')
        ...
        for index in range(self.NumBifurPara):
            ...
            if Predict == 1:
                ...
                distribution(Output,T_puptimes,T_pdowntimes,T_pupinterval,T_pdowninterval)
            
            if Predict == 0:
                plt.xlabel('$x$',fontsize=12)
                plt.ylabel('$y$',fontsize=12)
                plt.axvline(x=-1.12,c='black',ls='--')
                plt.axvline(x=1.12,c='black',ls='--')
                ...
                # plt.axhline(1.12,linestyle='--',color='navy',alpha=0.5)
                # plt.axhline(-1.12,linestyle='--',color='navy',alpha=0.5)
                ...
                # plt.xlim(0,500)
                # plt.ylim(-2,2)
                # plt.savefig('downxdeterministic.pdf',bbox_inches = 'tight')
                # plt.show()
            ...
            #     plt.xlim(-2,2)
            #     plt.ylim(-2,2)
            #     plt.savefig('downxyptra.svg',bbox_inches = 'tight')
            #     plt.show()
            ...
            #     plt.savefig('downxptra.pdf',bbox_inches = 'tight')
            #     plt.show()
            ...
            # plt.xlabel('$x$',fontsize=12)
            # plt.savefig('downxdnoise.pdf',bbox_inches = 'tight')
            # plt.show()
            # plt.hist(yNoiseToDraw, bins=bins, density=True, color=['coral','seagreen'])
            # plt.legend(['True','Predicted'],fontsize=12,loc='upper right')
            # plt.xlabel('$y$',fontsize=12)
            # plt.savefig('downydnoise.pdf',bbox_inches = 'tight')
            # plt.show()
...
if __name__ == '__main__':
    ...
    if UseDoubleWell==1:
        ...
        for i in range (1):
            ...
            rcpara[0]=1.3   #K_{in}
            rcpara[1]=3   #D
            rcpara[2]=4100   #relevant to rho: rho=0.00089
            rcpara[3]=0.6  #alpha
            rcpara[4]=1e-7 #beta
     
            paraTest=parascanRange[0]#0.2
            para[2]=paraTest#0.2# test parameter
            Utotal=[]
        ...
        if Predict == 1:
            draw_distribution(T_auptimes,T_puptimes, "Number of upward transitions", 'Number',bins_interval=1,margin=2)
            draw_distribution(T_adowntimes,T_pdowntimes, "Number of downward transitions", 'Number',bins_interval=1,margin=2)
            draw_distribution(T_aupinterval, T_pupinterval, "Upward transitions time step", 'Time step',bins_interval=50,margin=10)
            draw_distribution(T_adowninterval, T_pdowninterval, "Downward transition time step", 'Time step',bins_interval=50,margin=10)
