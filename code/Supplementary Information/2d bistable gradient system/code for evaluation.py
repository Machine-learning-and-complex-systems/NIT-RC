...
class Reservoir:
    ...
    def _run(self,WoutPara,Load,Predict):
        ...
        output=[[],[]]
        if Load==0:
            np.save(r'../data/2d bistable gradient system/xyWinw.npy',self.Win)
            np.save(r'../data/2d bistable gradient system/xyAw.npy',self.A)
            np.save(r'../data/2d bistable gradient system/xyWoutParaw.npy',WoutPara)            
            np.save(r'../data/2d bistable gradient system/xyWb.npy',self.Wb)
            np.save(r'../data/2d bistable gradient system/xyWxi.npy',self.Wxi)
            np.save(r'../data/2d bistable gradient system/xyr.npy',self.rFinalTrainTotal)
        else:
            self.Win=np.load(r'../data/2d bistable gradient system/xyWinw.npy')
            self.A=np.load(r'../data/2d bistable gradient system/xyAw.npy')
            WoutPara=np.load(r'../data/2d bistable gradient system/xyWoutParaw.npy')
            self.Wb=np.load(r'../data/2d bistable gradient system/xyWb.npy')
            self.Wxi=np.load(r'../data/2d bistable gradient system/xyWxi.npy')
            self.rFinalTrainTotal=np.load(r'../data/2d bistable gradient system/xyr.npy')
...
if __name__ == '__main__':
    ...
    if UseDoubleWell==1:
        ...
        for i in range (100):
            ...
            rcpara[0]=1.5   #K_{in}
            rcpara[1]=2   #D
            rcpara[2]=1500  #relevant to rho: rho=0.0016
            rcpara[3]=0.36  #alpha
            rcpara[4]=1e-7 #beta
