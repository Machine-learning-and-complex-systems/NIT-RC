...
dt = 0.001 #steplength
TestLength=50000#00 # Number of points for test/validation phase: 0.01 steplength
T2 =50+TestLength*dt  # Total time length: including training phase and test/validation phase
xanoise=[]
yanoise=[]
...
def Triplewell(gamma):
    ...
    for i in range(int(T2/dt)):
        u[0,i+1] = u[0,i] -dt * (-10*u[0,i]*np.exp(-u[0,i]**2)*(np.exp(-(u[1,i]-1/3)**2)-np.exp(-(u[1,i]-5/3)**2)) + 8*np.exp(-u[1,i]**2)*((u[0,i]-1)*np.exp(-(u[0,i]-1)**2)+(u[0,i]+1)*np.exp(-(u[0,i]+1)**2)) + 0.8*u[0,i]**3) 
        u[1,i+1] = u[1,i] -dt * (-10*np.exp(-u[0,i]**2)*((u[1,i]-1/3)*np.exp(-(u[1,i]-1/3)**2)-(u[1,i]-5/3)*np.exp(-(u[1,i]-5/3)**2))+8*u[1,i]*np.exp(-u[1,i]**2)*(np.exp(-(u[0,i]-1)**2)+np.exp(-(u[0,i]+1)**2))+0.8*(u[1,i]-1/3)**3)
        q[0,i] = np.sqrt(dt*2/gamma) * np.random.randn()
        q[1,i] = np.sqrt(dt*2/gamma) * np.random.randn()
    ...
    #plt.xlim(0,30)
    #plt.savefig('twatra.pdf',bbox_inches = 'tight')
    #plt.show()

    u_t=np.zeros((1,TestLength))
    u_t[0,:]=u[0,50001:int(T2/dt)+1]
    return u
...
class Reservoir:
    ...
    def _run(self,WoutPara,Load,Predict):
        ...
        output=[]
        if Load==0:
            np.save(r'../data/2d tristable system/twWinw.npy',self.Win)
            np.save(r'../data/2d tristable system/twAw.npy',self.A)
            np.save(r'../data/2d tristable system/twWoutParaw.npy',WoutPara)            
            np.save(r'../data/2d tristable system/twWb.npy',self.Wb)
            np.save(r'../data/2d tristable system/twWxi.npy',self.Wxi)
            np.save(r'../data/2d tristable system/twr.npy',self.rFinalTrainTotal)
        else:
            self.Win=np.load(r'../data/2d tristable system/twWinw.npy')
            self.A=np.load(r'../data/2d tristable system/twAw.npy')
            WoutPara=np.load(r'../data/2d tristable system/twWoutParaw.npy')
            self.Wb=np.load(r'../data/2d tristable system/twWb.npy')
            self.Wxi=np.load(r'../data/2d tristable system/twWxi.npy')
            self.rFinalTrainTotal=np.load(r'../data/2d tristable system/twr.npy')
        ...
        for index in range(self.NumBifurPara):
            ...
            if Predict == 0:
                ...
                plt.savefig('../results/prediction.pdf',bbox_inches = 'tight',dpi=400)
                plt.show()
            ...
            #plt.savefig('twdnoise.pdf',bbox_inches = 'tight')
            #plt.show()


def GenerateEnsemble(Realizations,para):
    for j in range(Realizations):
        uTemp=Triplewell(para) 
        if j==0:
            u=uTemp
        else:
            u=np.vstack((u, uTemp))
...
if __name__ == '__main__':
    ...
    if UseDoubleWell==1:
        ...
        for i in range (1):
            ...
            rcpara[0]=1   #K_{in}
            rcpara[1]=2.8   #D
            rcpara[2]=2000  #relevant to rho: rho=0.0017
            rcpara[3]=0.45  #alpha
            rcpara[4]=1e-6 #beta

            paraTest=parascanRange[0]#0.2   
            Utotal=[]
            u=GenerateEnsemble(Realizations,gamma)
            Utotal.append(u)
