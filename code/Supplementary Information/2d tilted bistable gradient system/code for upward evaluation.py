...
def DoubleWell(para):#Generate dataset
    ...
    #plt.scatter(u[0,:],u[1,:],c='coral',s=0.4)   
    #plt.xlim(-2,2)
    #plt.ylim(-2,2)
    #plt.xlabel('$x$',fontsize=12)
    #plt.ylabel('$y$',fontsize=12)
    #plt.show()
    #plt.savefig('upxyatra.pdf',bbox_inches = 'tight')
...
class Reservoir:
    ...
    def train(self,parascanRange):#To train W_{out}
        self.Win =np.random.uniform(-self.kin,self.kin, (self.N, self.M + 1))#Win#
        self.Wb= np.random.uniform(-self.kin, self.kin, (self.N, 1))#Wb#
        self.Wxi=np.random.uniform(-self.kin,self.kin, (self.N, self.M))#Wxi  
        ...
        R_T=R.T
        # Wout = (s * r^T) * ((r * r^T) + beta * I) #should have inverse?
    ...
    def _run(self,WoutPara,Load,Predict):
        ...
        output=[[],[]]
        if Load==0:
            np.save(r'../data/2d tilted bistable gradient system/upxyWinw.npy',self.Win)
            np.save(r'../data/2d tilted bistable gradient system/upxyAw.npy',self.A)
            np.save(r'../data/2d tilted bistable gradient system/upxyWoutParaw.npy',WoutPara)            
            np.save(r'../data/2d tilted bistable gradient system/upxyWb.npy',self.Wb)
            np.save(r'../data/2d tilted bistable gradient system/upxyWxi.npy',self.Wxi)
            np.save(r'../data/2d tilted bistable gradient system/upxyr.npy',self.rFinalTrainTotal)
        else:
            self.Win=np.load(r'../data/2d tilted bistable gradient system/upxyWinw.npy')
            self.A=np.load(r'../data/2d tilted bistable gradient system/upxyAw.npy')
            WoutPara=np.load(r'../data/2d tilted bistable gradient system/upxyWoutParaw.npy')
            self.Wb=np.load(r'../data/2d tilted bistable gradient system/upxyWb.npy')
            self.Wxi=np.load(r'../data/2d tilted bistable gradient system/upxyWxi.npy')
            self.rFinalTrainTotal=np.load(r'../data/2d tilted bistable gradient system/upxyr.npy')
...
if __name__ == '__main__':
    ...
    if UseDoubleWell==1:
        ...
        for i in range (100):
            ...
            rcpara[0]=1.6   #K_{in}
            rcpara[1]=3.6   #D
            rcpara[2]=3000  #relevant to rho: rho=0.0014
            rcpara[3]=0.58  #alpha
            rcpara[4]=1e-7 #beta
