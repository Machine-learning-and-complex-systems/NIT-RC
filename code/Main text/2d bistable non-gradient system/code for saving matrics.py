...
class Reservoir:
    ...
    def _run(self,WoutPara,Load,Predict):
        ...
        output=[[],[]]
        if Load==0:
            np.save(r'../results/xyWinw.npy',self.Win)
            np.save(r'../results/xyAw.npy',self.A)
            np.save(r'../results/xyWoutParaw.npy',WoutPara)            
            np.save(r'../results/xyWb.npy',self.Wb)
            np.save(r'../results/xyWxi.npy',self.Wxi)
            np.save(r'../results/xyr.npy',self.rFinalTrainTotal)
        else:
