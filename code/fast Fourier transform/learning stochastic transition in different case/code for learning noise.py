...
dt = 0.01; #steplength
startLength = 3660 # Initial point of transition occuring
TestLength = 400# Number of points for test/validation phase: 0.01 steplength
TrainLength= 2050
start = time.time() 
...
def DoubleWell():#Load data
    q=np.load('../data/fast Fourier transform/noise.npy')
    U=q.reshape(1,-1)
    length = np.shape(U)[1]
...
class Reservoir:
    ...
    def _run(self,WoutPara,i):
        ...
        for index in range(self.NumBifurPara):
            ...
            plt.savefig('Lptra.pdf',bbox_inches = 'tight')
            plt.show()
...
if __name__ == '__main__':
    ...
    if UseDoubleWell==1:
        ...
        for i in range (L):
            ...
            rcpara[1]=0.996  #D
            rcpara[2]=1.88   #relevant to rho: rho=0.806
