...
def DoubleWell(para):  #Generate dataset
    ...
    plt.savefig('../results/Savizky-Golay.pdf',bbox_inches = 'tight')
    plt.show()
...
class Reservoir:
    ...
    def _run(self,WoutPara,Load,Predict):
        ...
        for index in range(self.NumBifurPara):
            ...
            # if Predict ==1:
            #     distribution(self.S,T_ptimes,T_pinterval)
            ...
            for i in range(TestLength - 1):
                if Q == 5:
                    K[0,i+1] = K[0,i] + dt * (-0.0111 -  0.008 * K[0,i] **2 - 0.013* K[0,i]**3)
                elif Q == 10:
                    K[0,i+1] = K[0,i] + dt * (-0.0181 - 0.016 * K[0,i])
                else:
                    K[0,i+1] = K[0,i] + dt * (-0.0201 -  0.023 * K[0,i] + 0.006 * K[0,i]**2 +0.008 * K[0,i]**3)
