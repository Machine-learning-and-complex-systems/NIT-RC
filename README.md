...
For systems with colored noise (two Lorenz-63 and one Lorenz-96), we provide the predicted noise "NoiseTotal.npy" of 50 times predictions, and the matrices of the deterministic RC. To generate transition trajectories, run the provided script code_Total_for_predicting_phase.py. The prediction for FIG. 3(d) may differ slightly from the version in the paper (green line), due to the data for this figure in the arXiv version being inadvertently overwritten by Supplementary FIG. 20 (represent the same system with different parameters). Therefore, we provide a correct version of the prediction.
